// Cleaned up global dark mode for SOAP Generator
"use client";
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-teal-50 to-blue-100 dark:from-gray-900 dark:to-gray-950 py-8 px-4 sm:px-6 md:px-8">
      ...
      <div className="w-full max-w-2xl bg-white/30 dark:bg-gray-800/40 shadow-xl rounded-xl backdrop-blur-md p-6 sm:p-8 space-y-4">
        {[
          { val: signalment, set: setSignalment, placeholder: "🐶 Signalment (Age, Breed, Sex, etc.)" },
          { val: history, set: setHistory, placeholder: "📚 History" },
          { val: clinicalFindings, set: setClinicalFindings, placeholder: "🔍 Clinical Findings" },
          { val: assessment, set: setAssessment, placeholder: "🧠 Assessment" },
          { val: plan, set: setPlan, placeholder: "📝 Plan" },
        ].map((f, idx) => (
          <textarea
            key={idx}
