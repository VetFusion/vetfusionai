...
function splitSections(full?: string | null) {
  ...
  if (marks.length === 0) { if (src.trim()) blocks["SUMMARY"] = src.trim(); return blocks; }
...
function parseRun(text?: string | null) {
  ...
  if (m1) { const run = m1[1].toUpperCase(); const [ward, area] = run.split("-"); return { run, ward: ward ?? null, area: area ?? null, cage: null }; }
  const m2 = t.match(/\b(BR|MR|ICU|MC|CAT ICU|DOG ICU|MAIN|B)-?\s?(\d{1,3}[A-Z]?)\b/i);
  if (m2) { const wRaw = m2[1].toUpperCase(); const ward = wRaw === "B" ? "BR" : wRaw === "MAIN" ? "MR" : wRaw; const area = m2[2].toUpperCase(); return { run: `${ward}-${area}`, ward, area, cage: null }; }
...
function parseVitals(text?: string | null) {
  ...
  return { temp_f: temp_f ? Number(temp_f) : null, hr_bpm: hr_bpm ? Number(hr_bpm) : null, rr_bpm: rr_bpm ? Number(rr_bpm) : null };
}
function parseWeight(text?: string | null) {
  ...
  if (kg == null && lb != null) { kg = Math.round((lb + (oz ? oz/16 : 0)) * 0.453592 * 100) / 100; }
...
function parseMeds(text?: string | null) {
  ...
  for (const ln of lines) { const m = ln.match(rx); if (!m) continue; const [, drug, dose="", route="", freq="", notes=""] = m; if (!drug) continue;
    meds.push({ drug:drug.trim(), dose:dose.trim(), route:route.toUpperCase(), freq:freq.toUpperCase(), notes:notes.trim() }); }
  const uniq = new Map<string, typeof meds[number]>(); meds.forEach(row => uniq.set([row.drug,row.dose,row.route,row.freq,row.notes].join("|"), row));
  return Array.from(uniq.values());
}
function scrubSummary(text?: string | null) {
  if (!text) return "";
  const drop = [/^🐾.*DELTA RESCUE/i,/^Nonprofit Owner/i,/Arrastre Canyon/i,/\b\d{3}[-\s]?\d{3}[-\s]?\d{4}\b/,/^Decision-?Maker/i,/^VCPR/i,/^Vaccines/i,/^Other Vitals/i];
  return text.split("\n").map(s=>s.trim()).filter(ln => !drop.some(rx => rx.test(ln))).join("\n").trim();
}
function detectChips(texts: string[]) {
  const t = texts.join(" ").toLowerCase(); const chips: string[] = [];
...
function parseLabs(text?: string | null) {
  ...
  const get = (re: RegExp) => { const m = t.match(re); return m ? m[1] : undefined; };
  const out: Record<string,string> = {};
  const PCV=get(/\bPCV\b[^0-9\-]*([0-9]{1,2}(?:\.[0-9])?)/i); if(PCV) out.PCV=`${PCV} %`;
  const HCT=get(/\bHCT\b[^0-9\-]*([0-9]{1,2}(?:\.[0-9])?)/i); if(HCT) out.HCT=`${HCT} %`;
  const TS =get(/\b(?:TS|TP)\b[^0-9\-]*([0-9]{1,2}(?:\.[0-9])?)/i); if(TS) out.TS =`${TS} g/dL`;
  const BUN=get(/\bBUN\b[^0-9\-]*([0-9]{1,3})/i);              if(BUN) out.BUN=`${BUN} mg/dL`;
  const CREA=get(/\b(?:CREA|Creatinine)\b[^0-9\-]*([0-9]{1,2}(?:\.[0-9])?)/i); if(CREA) out.CREA=`${CREA} mg/dL`;
  const ALT=get(/\bALT\b[^0-9\-]*([0-9]{1,4})/i);               if(ALT) out.ALT=`${ALT} U/L`;
  const ALP=get(/\bALP\b[^0-9\-]*([0-9]{1,4})/i);               if(ALP) out.ALP=`${ALP} U/L`;
  const GLU=get(/\b(?:GLU|Glucose)\b[^0-9\-]*([0-9]{1,3})/i);   if(GLU) out.GLU=`${GLU} mg/dL`;
  const T4 =get(/\bT4\b[^0-9\-]*([0-9]{1,2}(?:\.[0-9])?)/i);    if(T4) out.T4 =`${T4} µg/dL`;
  const TP =get(/\bTotal\s*Protein\b[^0-9\-]*([0-9]{1,2}(?:\.[0-9])?)/i); if(TP) out.TP =`${TP} g/dL`;
  const ALB=get(/\b(?:ALB|Albumin)\b[^0-9\-]*([0-9]{1,2}(?:\.[0-9])?)/i);  if(ALB) out.ALB=`${ALB} g/dL`;
  return out;
}
  ...
  if (soapErr || !soap) {
    return (
      <div className="vf-glass vf-glow p-5 m-6">
        ...
        <a className="vf-btn mt-3 inline-block" href="/tracker">← Back to Tracker</a>
      </div>
    );
  }
...
TPR: ${tempF} °F / ${hrBpm} bpm / ${rrBpm} bpm`;
  const planText = S["PLAN"] ?? (medsParsed.length
    ...
    <div className="space-y-5 p-4 sm:p-6">
      ...
      <div className="vf-glass vf-glow p-3 flex items-center justify-between">
        <div className="text-sm text-gray-300">
          <a href="/tracker" className="underline">← Back to Tracker</a>
        </div>
      ...
      <section className="vf-glass vf-glow p-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            {ageBadge}
            <span className="text-sm text-gray-400">ID: <code className="text-gray-300">{id}</code></span>
          </div>
        </div>
        ...
        {chips.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-2">
            {chips.map(c => (
              <span key={c} className="px-2 py-1 rounded-full text-xs bg-white/10 border border-white/15">{c}</span>
            ))}
          </div>
        )}
